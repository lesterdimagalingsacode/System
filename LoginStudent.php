...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Get student user
    $stmt = $conn->prepare("SELECT u.user_id, u.password, u.failed_attempts, s.student_id, s.name 
                            FROM users u
    ...
    if ($stmt->num_rows == 1) {
        $stmt->bind_result($user_id, $hashedPassword, $current_attempts, $student_id, $name);
        $stmt->fetch();

        if (is_string($hashedPassword) && password_verify($password, $hashedPassword)) {
            // Login successful - reset failed attempts
            $reset_stmt = $conn->prepare("UPDATE users SET failed_attempts = 0, last_failed_attempt = NULL WHERE user_id = ?");
            ...
            if ($new_attempts >= MAX_ATTEMPTS) {
                ...
                echo "<script>alert('Invalid password. You have " . $remaining_attempts . " attempt(s) remaining before your account is temporarily locked.');</script>";
            }
        }
    } else {
        echo "<script>alert('No student account found with that email.');</script>";
    }
    $stmt->close();
}
  ...
  <main class="login-container">
    ...
    <div class="form-section">
      <div class="form-container">
        ...
        <form action="LoginStudent.php" method="post" class="login-form">
          <div class="input">
...
-->
          ...
          <div class="form-footer">
            <p class="signup-text">
              New here? 
              <a href="studentAdd.php" class="signup-link">Create your student account</a>
            </p>
          </div>
        </form>
      </div>
